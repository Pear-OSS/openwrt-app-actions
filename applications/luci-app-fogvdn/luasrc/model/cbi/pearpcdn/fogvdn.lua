...
"Note: The OpenFog service must not be operated concurrently with peer services in the same category.")
...
openfog_link = s:option(DummyValue, "openfog_link",
    translate(
    "<input type=\"button\" class=\"cbi-button cbi-button-apply\" style=\"text-transform: none !important;\" value=\"openfogos.com\" onclick=\"window.open('https://openfogos.com/')\" />"))
...
if tutorial_url then
    tutorial_link = s:option(DummyValue, "tutorial_link",
        "<input type=\"button\" class=\"cbi-button cbi-button-apply\" value=\"" ..
        translate("Tutorial") .. "\" onclick=\"window.open('" .. tutorial_url .. "')\" />")
...
"<input type=\"button\" class=\"cbi-button cbi-button-apply\" value=\"Register\" onclick=\"window.open('https://account.openfogos.com/signup?source%3Dopenfogos.com%26')\" />")
...
function per_line_up_bw.validate(self, value, section)
    ...
    if value and value ~= "" then
        ...
        if not num_value then
            per_line_up_bw.description = [[<b style="color: red">]] ..
            translate("ERROR: Invalid number format") .. "</b>"
            return nil
        end
        ...
        if num_value < min_upload_speed then
            local arch = get_cpu_arch()
            per_line_up_bw.description = [[<b style="color: red">]] ..
            translate("ERROR: Upload Speed must more than " .. min_upload_speed .. " Mbps") .. "</b>"
            return nil
        end
    end
...
limited_memory.datatype = "range(20, 100)"
-- 0-100%
...
limited_storage.datatype = "range(20, 100)"
-- 0-100%

limited_area = s:option(Value, "limited_area", translate("Limited Area"))
limited_area.default = "2"
limited_area:value("-1",
    translate(
    "Unset: If you are unsure about the operator's restrictions on inter-provincial traffic, it is recommended to choose this option."))
limited_area:value("0",
    translate(
    "Domestic: Traffic will be scheduled nationwide, resulting in a higher proportion of inter-provincial traffic, which may lead to restrictions by the operator."))
limited_area:value("1",
    translate(
    "Provincial: Traffic will only be scheduled within the province where it is located, which is a safer scheduling mode, but the volume of traffic may decrease."))
limited_area:value("2",
    translate(
    "Regional: Traffic will only be scheduled within the region where it is located, resulting in a lower proportion of inter-provincial traffic. This mode provides a balanced approach between safety and traffic volume."))
...
storage.description = translate(
"Please prioritize solid-state drives with integrated DRAM cache and write-through capability, preferably meeting enterprise-grade specifications.")
